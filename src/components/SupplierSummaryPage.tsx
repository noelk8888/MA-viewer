...
export const SupplierSummaryPage: React.FC<{ onBack: () => void; onMonthClick: (month: string) => void }> = ({ onBack, onMonthClick }) => {
  ...
  return <div className="w-full max-w-2xl mx-auto bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-100 my-4 sm:my-8">
    ...
    {loading ? <div className="py-20 text-center text-gray-400">Loading supplier data...</div> : error ? <div className="py-20 text-center text-red-500">{error}</div> : <div className="text-[15px]"><div className="grid grid-cols-4 border-b border-gray-300 bg-gray-50 font-bold"><div className="p-3">MONTH</div><div className="p-3 text-right">AMOUNT</div><div className="p-3 text-right">JKB</div><div className="p-3 text-right">NCK</div></div>{data.map((item) => <button key={item.label} onClick={() => onMonthClick(item.label)} className="grid grid-cols-4 w-full text-left border-b border-gray-100 hover:bg-blue-50"><div className="p-3 font-medium">{item.label}</div><div className="p-3 text-right">{fmt(item.amount)}</div><div className="p-3 text-right">{fmt(item.jkb)}</div><div className="p-3 text-right">{fmt(item.nck)}</div></button>)}<div className="grid grid-cols-4 bg-gray-50/50 border-t-2 border-gray-900 font-bold"><div className="p-3">TOTAL</div><div className="p-3 text-right">{fmt(total.amount)}</div><div className="p-3 text-right">{fmt(total.jkb)}</div><div className="p-3 text-right">{fmt(total.nck)}</div></div></div>}
  </div>;
};
